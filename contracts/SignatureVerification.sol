...
contract SignatureVerification {
    ...
    function retrieveSenderSignaturesTxIds(
        address _senderAddress
    ) external view returns (uint[] memory) {
        return sender[_senderAddress];
    }

    function retrieveRecieverSignaturesTxIds(
    ...
    function verify(
        address _signer,
        string memory _message,
        bytes memory _sig //address from sig and address provided are they equal ?
    ...
    function getSigner(
        ...
        return recover(ethSignedMessageHash, _sig); // returns signer address
    }
